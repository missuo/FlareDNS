...
@interface CFDomainsListViewController () <UITableViewDelegate, UITableViewDataSource, UISearchBarDelegate, UISearchResultsUpdating, CFAccountsViewControllerDelegate>
...
- (void)setupUI {
    ...
    [self.tableView registerClass:[CFDomainCell class] forCellReuseIdentifier:kDomainCellIdentifier];
    [self.view addSubview:self.tableView];
    
    // Refresh control
    self.refreshControl = [[UIRefreshControl alloc] init];
    [self.refreshControl addTarget:self action:@selector(loadZones) forControlEvents:UIControlEventValueChanged];
...
- (void)addButtonTapped {
    UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"Add Domain"
                                                                   message:@"Enter the domain name you want to add to Cloudflare"
    ...
    [alert addAction:[UIAlertAction actionWithTitle:@"Add" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        NSString *domainName = alert.textFields.firstObject.text;
...
- (void)addDomainWithName:(NSString *)domainName {
    ...
    [[CFAPIService shared] addZoneWithName:domainName completion:^(CFZone * _Nullable zone, NSError * _Nullable error) {
        [self.activityIndicator stopAnimating];
        ...
        NSString *message = [NSString stringWithFormat:@"Domain '%@' has been added successfully.\n\nPlease update your domain's nameservers to:\n%@", 
                            zone.name, 
...
- (void)settingsButtonTapped {
    CFAccountsViewController *settingsVC = [[CFAccountsViewController alloc] init];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    CFDomainCell *cell = [tableView dequeueReusableCellWithIdentifier:kDomainCellIdentifier forIndexPath:indexPath];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    CFZone *zone = self.filteredZones[indexPath.row];
    CFDomainDetailViewController *detailVC = [[CFDomainDetailViewController alloc] initWithZone:zone];
