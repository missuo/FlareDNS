...
- (void)drawRect:(CGRect)rect {
  // Draw a flat horizontal line when there is no (or insufficient) data
  if (self.dataPoints.count < 2) {
    ...
    UIBezierPath *flatLine = [UIBezierPath bezierPath];
    [flatLine moveToPoint:CGPointMake(4, midY)];
    [flatLine addLineToPoint:CGPointMake(rect.size.width - 4, midY)];
    flatLine.lineWidth = 1.0;
    [[self.lineColor colorWithAlphaComponent:0.5] setStroke];
    [flatLine stroke];
    return;
  }
  ...
  NSArray *colors = @[
    (__bridge id)[UIColor cf_chartGradientStartColor].CGColor,
